...
#define LIFT_SPEED 40//Speed for lifting the A-frame when finishing a profile
...
double straightResistance = 790; //ohms - rod straight (no tension) (Daywa Custom Design (Dark Grey) - 800)
const double stopResistance = 0; //ohms - position of rod to send stop command (getting too slack) (should be about 722)
double bentResistance = 651; //ohms - rod bent (full tension) (should be about 620)
double desiredResistance = 700; //ohms - resistance to attempt to maintain at all times
...
const double RAMP_TIME = 1000; //Time it takes to change speed in milliseconds
  ...
  else
  {
      if(newDir == DOWN) 
      {
         ...
         /* for(int i = 0; i < 100; i++) //averages the increase value 
          ...
          resistance = analogRead(resistancePin);
          float range1 = desiredResistance + 20;
          ...
          rodPID.Compute();  // outputs a new speedTry
          if(analogRead(resistancePin > (straightResistance - 30) )){
