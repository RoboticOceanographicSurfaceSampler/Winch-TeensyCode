/*    ROSS Auxillary Board Control Code
 *  
 *  This code handles the control function of the Auxillary Board in the ROSS Ebox.
 *  
 *  Made for the ROSS Project at OSU.
 * 
 *  It's primary functions are as follows:
 *  - Controls the ROSS Winch (Outputs PWM)
 *  - Reads from the Hall Effect Sensors that determine A-Frame position
 *  - Reads from an Incremental Encoder that determines winch postition
 *  - Controls relays that enable the kayak to start and stop it's engine
 * 
 *  Code owner: Jasmine Nahorinak (EMAIL)
 * 
 *  Code Authors (alpha)
 *  - Jorian Brushlind
 *  - Nick McComb 
 ...
 *  - Robert Shannon
 * 
 *  For PCB Schematic, visit www.nickmccomb.net/pcb
 * 
 **/
...
- Added LED Indicators for Hall Effect Sensors
- Added code neatness tweaks
...
//#define LINEAR  //One of these must be defined to determine the ramping type 
...
struct Winch_TYPE { //Assigns variables to the object "winch" (Ex, winch.currentSpeed is now a variable, as is winch.prevSpeed)
  double currentSpeed;
  double prevSpeed;
  uint8_t currentDir; // UP,DOWN,ST\OP defined in enum
  uint8_t prevDir;
...
//Define sensor pins
#define downPin A4 //Hall Effect sensor indicating lowered position (no longer used) (digital 17) (S1)
#define downLED 14 //Green status LED on PCB
#define upPin A3 //Hall Effect sensor indicating upright position (digital 18) (S2)
#define upLED 15   //Red error LED
...
void setup() 
{
    ...
    //Initialize pin states so we don't accidentially start the boat
    ...
    //Send a status message every X milliseconds
    statusTimer.every(statusFrequency, sendStatus);

    //Setup the winch control object
    ...
    //Setup the ROD PID control object
    ...
    // ** Begin effecting environment ** //

    while(atTop() == false) //waits for the A-frame to be all the way up (IN SETUP FUNCTION, THIS WILL WAIT TO START MAIN PROGRAM UNTIL THIS IS TRUE)
    { 
...
bool atTop(){
  ...
  //Added by Nick for Debugging and Hall Effect setup
  digitalWrite(upLED,   digitalRead(upPin));   //red error
  digitalWrite(downLED, digitalRead(downPin)); //green status
