...
#define MAINTAIN_SPEED 50 //Speed for lifting the A-frame when maintaining
...
//Define sensor pins
#define downPin A3//Hall Effect sensor indicating lowered position (no longer used)
#define downLED 13
#define upPin A4 //Hall Effect sensor indicating upright position
#define upLED 15
  ...
  else
  {
      if(newDir == DOWN) 
      {
         ...
         /* for(int i = 0; i < 100; i++) //averages the increase value 
          ...
          if(analogRead(resistancePin > straightResistance - (straightResistance - desiredResistance)*.2 ))
          {
          ...
          speedOut = constrain(speedOut, 7, 40);
          //delay(50);
