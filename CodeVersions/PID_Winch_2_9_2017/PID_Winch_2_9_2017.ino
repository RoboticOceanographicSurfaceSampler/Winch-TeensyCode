...
int speedOut = 20; //variable from MatLab
int speedIn = 20; //variable from MatLab
double speedTry = 0; //Variable to store the PID output
...
void changeSpeed(uint8_t newSpeed, uint8_t newDir)  {
  ...
  {  //If the command is to continue moving the same speed and direction then don't do the sinusoid
    ...
    if(kickstart == true) 
    {
      ...
      if(newDir == UP)
      {
      winch.currentSpeed = newSpeed*(deltaT/ramp) + 100; //will set newspeed to ramp up to the set speed in .5 secs (500 millis)
      }
      else if(newDir == DOWN)
      {
